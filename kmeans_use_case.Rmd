...
```{r}
set.seed(100)
index <- sample(nrow(paysim), nrow(paysim)*0.01)
paysim_subset <- paysim[index,]
```


```{r}
pay_coba <- paysim_subset %>% 
  filter(Fraud == "Yes")
```
...
e) Insp: Contains the report of three possible values i.e. ok, fraud and unknown
...
set.seed(100)
sales.km <- kmeans(sales_num, centers = 3)
```
...
```{r}
ggplot(sales_baru, aes(x = Quant, y = Val)) +
  geom_point(aes(col = as.factor(sales.km$cluster), shape = sales_baru$Insp))
```
...
```{r}
sales_baru$cluster <- sales.km$cluster
```

```{r}
sal_filter <- sales_baru %>% 
  filter(Insp == "fraud")
```


```{r}
table(sales_baru$Insp, sales_baru$cluster)
```
