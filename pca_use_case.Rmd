...
Here, we do the several preprocessing step using `recipe()` function from [recipe](https://tidymodels.github.io/recipes/) package. We store all of our preprocessing, in `step_*()` function, including **PCA** step. Here, the syntax of PCA is `step_pca(all_numeric(), threshold = 0.90)` we use the numeric variable only, and take the 90% of cummulative variance of the data, hence the threshold is set by 0.90.
